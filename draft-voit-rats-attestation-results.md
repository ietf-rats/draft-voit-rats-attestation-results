...
Frequently, this action will be to choose whether to allow the Attester to securely interact with the Relying Party over a connection between the two.

When determining whether to allow connectivity-based secure interactions with an Attester, a Relying Party is challenged with a number of difficult problems which it must be able to handle successfully.
...
This is not a reasonable ask as there can be many types of Attesters connecting into a Relying Party.

The business need therefore is for common Attestation Result information element definitions.
With these definitions, consistent connectivity decisions can be made by a Relying Party where there is a heterogenous mix of Attesting Environment types and Verifier types.
...
Extensions to this document can enable additional TEE environments and additional information elements to be supported.
...
These Attestation Results must include a Trustworthiness Vector describing a Verifier's most recent appraisal of an Attester, and some Verifier Proof-of-Freshness (PoF).
...
In this case a Relying Party will simply will accept a connection request from any device successfully appraised and verified by a Verifier.
...
An example of implicit communication would be to include the following Identity Evidence: a signature over all Attestation Results generated by a Verifier.
...
### Specific Claims
A Verifier must be able to assert different aspects of Attester trustworthiness.
...
Example mappings for SGX, Trustzone, and TPMs can be seen in {{claim-for-TEE-types}}.
...
| secure-storage | Minimal secure storage space exists and is writeable by external applications.
This space would typically just be used to store keys. |
